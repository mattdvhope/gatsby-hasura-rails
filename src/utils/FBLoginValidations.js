...
console.log("params: ", params)
  const response = await fetch(`https://graph.facebook.com/v10.0/oauth/access_token?${params}`)
...
export async function inspectAccessToken(token, appToken) {
  ...
  const response = await fetch(`https://graph.facebook.com/v10.0/debug_token?${params}`)
  const json = await response.json();
...
export async function getUserProfile(user_id, token) {
  const response = await fetch(`https://graph.facebook.com/v10.0/${user_id}?fields=id,name,first_name,middle_name,last_name,name_format,picture,short_name,email&access_token=${token}`)
  const json = await response.json();
  return json;
}